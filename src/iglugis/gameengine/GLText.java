...
public class GLText {
   ...
   //--Members--//
   GLGraphics glGraphics;                                           // GL10 Instance
   ...
   Texture tex;                                     // Font Texture ID [NOTE: Public for Testing Purposes Only!]
   ...
   public GLText(GLGame glgame, GLGraphics glGraphics) {
      this.glGraphics = glGraphics;                                   // Save the GL10 Instance
      this.glgame = glgame;

      batch = new SpriteBatch( glGraphics, CHAR_BATCH_SIZE );  // Create Sprite Batch (with Defined Size)
   ...
   public boolean load(String file, int size, int padX, int padY) {
      ...
      Typeface tf = Typeface.createFromAsset( glgame.getAssets(), file );  // Create the Typeface from Font File
      ...
      fontHeight = (float)android.util.FloatMath.ceil( Math.abs( fm.bottom ) + Math.abs( fm.top ) );  // Calculate Font Height
      fontAscent = (float)android.util.FloatMath.ceil( Math.abs( fm.ascent ) );  // Save Font Ascent
      fontDescent = (float)android.util.FloatMath.ceil( Math.abs( fm.descent ) );  // Save Font Descent
      ...
      rowCnt = (int)android.util.FloatMath.ceil((float)CHAR_CNT / (float)colCnt );  // Calculate Number of Rows
      ...
      tex = new Texture(glgame,bitmap);
//      int[] textureIds = new int[1];                  // Array to Get Texture Id
//      glGraphics.getGL().glGenTextures( 1, textureIds, 0 );           // Generate New Texture
//      textureId = textureIds[0];                      // Save Texture Id

      // setup filters for texture
      tex.bind();
      tex.setFilters(GL10.GL_NEAREST, GL10.GL_LINEAR);
      tex.setWrap(GL10.GL_CLAMP_TO_EDGE,GL10.GL_CLAMP_TO_EDGE);
      ...
      for ( int c = 0; c < CHAR_CNT; c++ )  {         // FOR Each Character (On Texture)
         charRgn[c] = new TextureRegion( tex, x, y, cellWidth-1, cellHeight-1 );  // Create Region for Character
      ...
      textureRgn = new TextureRegion( tex, 0, 0, textureSize, textureSize );  // Create Full Texture Region
   ...
   public void begin(float red, float green, float blue, float alpha)  {
	   glGraphics.getGL().glColor4f( red, green, blue, alpha );        // Set Color+Alpha
	   tex.bind();
      batch.beginBatch();                             // Begin Batch
   }
   public void end()  {
      batch.endBatch();                               // End Batch
      glGraphics.getGL().glColor4f( 1.0f, 1.0f, 1.0f, 1.0f );         // Restore Default Color/Alpha
   }
