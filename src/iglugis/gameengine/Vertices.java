...
public class Vertices {
   ...
   final GLGraphics glGraphics;                                     // GL Instance
   ...
   public Vertices(GLGraphics glGraphics, int maxVertices, int maxIndices, boolean hasColor, boolean hasTexCoords, boolean hasNormals)  {
      this( glGraphics, maxVertices, maxIndices, hasColor, hasTexCoords, hasNormals, false );  // Call Overloaded Constructor
   }
   public Vertices(GLGraphics glGraphics, int maxVertices, int maxIndices, boolean hasColor, boolean hasTexCoords, boolean hasNormals, boolean use3D)  {
      this.glGraphics = glGraphics;                                   // Save GL Instance
   ...
   // R: [none]
   public void bind()  {
	   glGraphics.getGL().glEnableClientState( GL10.GL_VERTEX_ARRAY ); // Enable Position in Vertices
      vertices.position( 0 );                         // Set Vertex Buffer to Position
      glGraphics.getGL().glVertexPointer( positionCnt, GL10.GL_FLOAT, vertexSize, vertices );  // Set Vertex Pointer

      if ( hasColor )  {                              // IF Vertices Have Color
    	  glGraphics.getGL().glEnableClientState( GL10.GL_COLOR_ARRAY );  // Enable Color in Vertices
         vertices.position( positionCnt );            // Set Vertex Buffer to Color
         glGraphics.getGL().glColorPointer( COLOR_CNT, GL10.GL_FLOAT, vertexSize, vertices );  // Set Color Pointer
      }

      if ( hasTexCoords )  {                          // IF Vertices Have Texture Coords
    	  glGraphics.getGL().glEnableClientState( GL10.GL_TEXTURE_COORD_ARRAY );  // Enable Texture Coords in Vertices
         ...
         glGraphics.getGL().glTexCoordPointer( TEXCOORD_CNT, GL10.GL_FLOAT, vertexSize, vertices );  // Set Texture Coords Pointer
      }

      if ( hasNormals )  {
    	  glGraphics.getGL().glEnableClientState( GL10.GL_NORMAL_ARRAY );  // Enable Normals in Vertices
         ...
         glGraphics.getGL().glNormalPointer( GL10.GL_FLOAT, vertexSize, vertices );  // Set Normals Pointer
      }
   }
   ...
   public void draw(int primitiveType, int offset, int numVertices)  {
      if ( indices != null )  {                       // IF Indices Exist
         ...
         glGraphics.getGL().glDrawElements( primitiveType, numVertices, GL10.GL_UNSIGNED_SHORT, indices );  // Draw Indexed
      }
      else  {                                         // ELSE No Indices Exist
    	  glGraphics.getGL().glDrawArrays( primitiveType, offset, numVertices );  // Draw Direct (Array)
      }
   }
   ...
   public void unbind()  {
      if ( hasColor )                                 // IF Vertices Have Color
    	  glGraphics.getGL().glDisableClientState( GL10.GL_COLOR_ARRAY );  // Clear Color State

      if ( hasTexCoords )                             // IF Vertices Have Texture Coords
    	  glGraphics.getGL().glDisableClientState( GL10.GL_TEXTURE_COORD_ARRAY );  // Clear Texture Coords State

      if ( hasNormals )                               // IF Vertices Have Normals
    	  glGraphics.getGL().glDisableClientState( GL10.GL_NORMAL_ARRAY );  // Clear Normals State
   }
